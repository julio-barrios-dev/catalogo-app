...
export default function CatalogoClient({ designs }: { designs: Design[] }) {
  ...
  return (
    <div className="min-h-dvh bg-white">
      ...
      <header className="py-10 px-6 text-center bg-white">
        ...
        <div className="mt-6 inline-flex flex-col sm:flex-row items-center gap-3 bg-amber-50 border border-amber-100 rounded-2xl px-5 py-4 max-w-xl mx-auto">
          <p className="text-sm text-gray-600 text-center sm:text-left">
            Seleccioná un diseño del catálogo, o{" "}
            <span className="font-semibold text-gray-800">
              creamos uno completamente desde cero
            </span>{" "}
            para vos.
          </p>
          <a
      ...
      {/* Floating custom-design WhatsApp button — always visible */}
      <a
        href={`https://wa.me/5491137781024?text=${encodeURIComponent("Hola! Me gustaría consultar sobre un diseño personalizado desde cero.")}`}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 left-6 z-20 bg-green-500 hover:bg-green-600 active:bg-green-700 text-white rounded-full px-4 py-3 shadow-xl transition-all duration-200 hover:scale-105 active:scale-95 flex items-center gap-2"
